...
function* BackendSaga(): SagaIterator {
    ...
    yield takeEvery(FETCH_COURSE_CONFIG, function* () {
        ...
        const { config }: { config: CourseConfiguration | null } = yield call(
            getCourseConfig,
            tokens
        )
        if (config) {
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.submitAnswer>): any {
            ...
            return yield put(
                actions.updateHasUnsavedChanges('assessment' as WorkspaceLocation, false)
            )
        }
    )
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.acknowledgeNotifications>): any {
            const tokens: Tokens = yield selectTokens()
            const notificationFilter: NotificationFilterFunction | undefined =
                action.payload.withFilter
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.deleteSourcecastEntry>): any {
            ...
            if (sourcecastIndex) {
                yield put(
                    actions.updateSourcecastIndex(sourcecastIndex, action.payload.workspaceLocation)
                )
            }
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.saveSourcecastData>): any {
            const [role, courseId]: [Role, number | undefined] = yield select(
                (state: OverallState) => [state.session.role!, state.session.courseId]
            )
            if (role === Role.Student) {
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.addNewUsersToCourse>): any {
            const tokens: Tokens = yield selectTokens()
            const { users, provider }: { users: UsernameRoleGroup[]; provider: string } =
                action.payload
