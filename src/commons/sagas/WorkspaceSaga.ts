import { isSourceLanguage, OverallState, styliseSublanguage } from '../application/ApplicationTypes'
...
export default function* WorkspaceSaga(): SagaIterator {
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.toggleEditorAutorun>): any {
            ...
            yield call(
                showWarningMessage,
                'Autorun ' + (isEditorAutorun ? 'Started' : 'Stopped'),
                750
            )
        }
    )
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.externalLibrarySelect>) {
            const { workspaceLocation, externalLibraryName: newExternalLibraryName } =
                action.payload
            ...
            if (newExternalLibraryName !== oldExternalLibraryName || action.payload.initialise) {
                ...
                if (!action.payload.initialise) {
                    yield call(
                        showSuccessMessage,
                        `Switched to ${newExternalLibraryName} library`,
                        1000
                    )
                }
            }
    ...
    yield takeEvery(
        ...
        function* (action: ReturnType<typeof actions.beginClearContext>) {
            ...
            EnvVisualizer.clear()
            const globals: Array<[string, any]> = action.payload.library.globals as Array<
                [string, any]
            >
    ...
    if (remoteExecutionSession && remoteExecutionSession.workspace === workspaceLocation) {
        ...
        if (!context.errors.length) {
            ...
            const breakpoints: string[] = yield select(
                // TODO: Hardcoded to make use of the first editor tab. Rewrite after editor tabs are added.
                (state: OverallState) =>
                    state.workspaces[workspaceLocation].editorTabs[0].breakpoints
            )
