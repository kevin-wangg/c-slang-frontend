...
const SideContentRemoteExecution: React.FC<SideContentRemoteExecutionProps> = props => {
    ...
    const isLoggedIn = useTypedSelector(
        state => !!state.session.accessToken && !!state.session.role
    )
                        ...
                        {devices?.map(device => {
                            ...
                            return (
                                <MenuItem
                                    key={id}
                                    onClick={() =>
                                        dispatch(actions.remoteExecConnect(props.workspace, device))
                                    }
                                    text={`${title} (${type})`}
                                    ...
                                    labelElement={
                                        <DeviceMenuItemButtons
                                            text={
                                                isSelected && isConnected ? 'Connected' : undefined
                                            }
                        ...
                        <div className="sa-remote-execution row">
                            {motorPorts.map(port => {
                                ...
                                return portData ? (
                                    ...
                                    <PeripheralContainer
                                        src={<PortSvg port={port.substring(port.length - 1)} />}
                                    />
                                )
                            })}
                        </div>
                        ...
                        <div className="sa-remote-execution row">
                            {sensorPorts.map(port => {
                                ...
                                return portData ? (
                                    <PeripheralContainer
                                        src={ev3PeripheralToComponentMap[portData.type]}
                                        text={ev3SensorModeToValueTransformerMap[portData.mode](
                                            portData.value
                                        )}
                                    />
                                ) : (
