...
const FileExplorerDialog: React.FC<FileExplorerDialogProps> = props => {
    ...
    async function setFirstLayerRepoFiles(repoName: string, setRepoFiles: any) {
        try {
            const initialRepoFiles = await GitHubTreeNodeCreator.getFirstLayerRepoFileNodes(
                repoName
            )
    ...
    async function handleSubmit() {
        ...
        if (props.pickerType === 'Open') {
            if (
                await checkIfFileCanBeOpened(props.octokit, githubLoginID, props.repoName, filePath)
            ) {
        ...
        if (props.pickerType === 'Save') {
            ...
            if (canBeSaved) {
                if (
                    saveType === 'Overwrite' &&
                    (await checkIfUserAgreesToPerformOverwritingSave())
                ) {
                    performOverwritingSave(
                        props.octokit,
                        githubLoginID,
    ...
    async function handleNodeClick(
        ...
        const allNodesCallback = (node: TreeNodeInfo<GitHubFileNodeData>) =>
