...
describe('BROWSE_REPL_HISTORY_DOWN', () => {
    test('works on non-null browseIndex and returns replValue to last value on no further records', () => {
        ...
        const replDownDefaultState: WorkspaceManagerState = generateDefaultWorkspace({
            replHistory
        })
...
describe('END_DEBUG_PAUSE', () => {
    test('sets isRunning to false and isDebugging to true', () => {
        const isRunning = true
        const debugPauseDefaultState: WorkspaceManagerState = generateDefaultWorkspace({
            isRunning
        })
        ...
        actions.forEach(action => {
            ...
            expect(result).toEqual({
                ...
                [location]: {
                    ...defaultWorkspaceManager[location],
                    editorTabs: [
                        { ...defaultWorkspaceManager[location].editorTabs[0], highlightedLines }
                    ]
                }
            })
        })
        ...
        actions.forEach(action => {
            ...
            expect(result).toEqual({
                ...
                [location]: {
                    ...
                    editorTabs: [
                        {
                            ...defaultWorkspaceManager[location].editorTabs[0],
