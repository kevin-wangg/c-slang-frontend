        ...
        <div className="Academy" style={{ overflow: 'hidden' }}>
            ...
            <Switch>
                <Route
                    ...
                    render={() => {
                        return octokit &&
                            (!courses || (courses.length > 0 && !assessmentTypeOverviews)) ? (
                            <ContentDisplay
                                display={
                                    <NonIdealState description="Loading..." icon={<Spinner />} />
                                }
                                loadContentDispatch={() => {}}
                            />
...
async function fetchAssessmentOverviews(
    ...
    if (Array.isArray(files)) {
        if (files.find(file => file.name === 'course-info.json')) {
            ...
            courseInfo.types.forEach(
                (type: { typeName: string; assessments: [GitHubAssessment] }) => {
