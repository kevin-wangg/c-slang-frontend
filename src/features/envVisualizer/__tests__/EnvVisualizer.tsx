        ...
        const checkNonCompactLayout = () => {
            ...
            Layout.values.forEach(v => {
                if (v instanceof GlobalFnValue || v instanceof FnValue) {
                    ...
                    expect(Frame.lastXCoordBelow(v.x())).toEqual(
                        Frame.lastXCoordBelow(arrow!.target!.x())
                    ) // target
                    ...
                    v.arrows().forEach(arrow => {
                        ...
                        if (
                            ...
                            if (sourceArray.level === targetArray.level) {
                                ...
                                const path = arrow!.path().match(/[^ ]+/g) ?? []
                                expect(parseFloat(path[1])).toEqual(
                                    arrow.source.x() + Config.DataUnitWidth / 2
                                )
                                expect(parseFloat(path[2])).toEqual(
                                    arrow.source.y() + Config.DataUnitHeight / 2
                                )
