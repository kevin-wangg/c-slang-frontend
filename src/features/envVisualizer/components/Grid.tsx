        ...
        nodes.forEach(node => {
            ...
            if (node[1].parent) {
                ...
                while (updatedParent) {
                    ...
                    if (
                        parentNode.xCoord &&
                        parentNode.children.flatMap(x => getNextChildren(x as EnvTreeNode))[0] ===
                            currentNode
                    ) {
                            ...
                            if (
                                ...
                                grid[level]
                                    ...
                                    .every(
                                        x =>
                                            x === undefined ||
                                            parseInt(x.parent.environment.id ?? 0) <
                                                parseInt(parentNode.environment.id)
                                    )
                            ) {
            ...
            (res, b, i) => {
                ...
                if (i % 2 === 0) {
                    ...
                    if ((b as ArrayLevel).count() > 0) {
                        if (
                            Frame.cumWidths[frameLevel.lastXcoord + 1] <= (b as ArrayLevel).minX()
                        ) {
                            height = Math.max(
                                height,
