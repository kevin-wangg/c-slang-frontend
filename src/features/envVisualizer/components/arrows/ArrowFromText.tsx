...
export class ArrowFromText<V extends Value> extends GenericArrow<Text, V> {
    protected calculateSteps() {
        ...
        if (target instanceof ArrayValue) {
            ...
            steps.push((x, y) => {
                ...
                return newX < x
                    ? [
                          x,
                          y -
                              Math.sign(source.y() - target.y()) *
                                  (source.height() + Config.TextMargin)
                      ]
                    : [x, y]
            ...
            if (
                (potentialHorizontalLaneY - source.y()) * (potentialHorizontalLaneY - target.y()) <
                0
            ) {
                steps.push((x, y) => {
                    const newY = ArrowLane.getHorizontalLaneBeforeTarget(target, y).getPosition(
                        target
                    )
