    ...
    protected calculateSteps() {
        ...
        if (target instanceof FnValue || target instanceof GlobalFnValue) {
            if (Math.abs(target.y() - source.y()) < Config.DataUnitHeight * 2) {
                ...
                steps.push(() => [
                    target.centerX + Config.FnRadius * 2 + Config.FnRadius,
                    target.y()
                ])
            ...
            if (source.parent === target) {
                ...
                if (
                    (potentialVerticalLaneX - source.x()) * (potentialVerticalLaneX - target.x()) <
                    0
                ) {
                ...
                if (
                    (potentialHorizontalLaneY - source.y()) *
                        (potentialHorizontalLaneY - target.y()) <
                    0
                ) {
                    steps.push((x, y) => {
                        return (potentialHorizontalLaneY - y) *
                            (potentialHorizontalLaneY - target.y()) >
                            0
                            ? [x, y]
                            : [
                                  x,
                                  ArrowLane.getHorizontalLaneBeforeTarget(target, y).getPosition(
                                      target
                                  )
                              ]
                    })
                ...
                steps.push((x, y) => {
                    ...
                    const newY =
                        ...
                        (Math.sign(yDiff) * Config.DataUnitHeight) / 2
                    yDiff = newY - y
